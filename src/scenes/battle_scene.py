...
class BattleScene(Scene):
    ...
    def __init__(self):
        ...
        self.dmg = 20  
        # 戰鬥中用到的 monster 資料
        self.player_mon: dict | None = None
        self.enemy_mon: dict | None = None
        ...
        self.enemy_anim_idle: BattleAnimation | None = None
        self.enemy_anim_attack: BattleAnimation | None = None
        self.enemy_anim: BattleAnimation | None = None

        # 對應的大圖片 / 小 icon
        self.player_sprite: pg.Surface | None = None
        self.enemy_sprite: pg.Surface | None = None
        # self.player_anim: dict[str, BattleAnimatedSprite] = {} # 儲存 { "idle": sprite, "attack": sprite }
        # self.enemy_anim: dict[str, BattleAnimatedSprite] = {}
        self.player_icon_small: pg.Surface | None = None
        self.enemy_icon_small: pg.Surface | None = None

        #icon
        # base_player_img = load_img(self.player_mon["sprite_path"])
        # base_enemy_img  = load_img(self.enemy_mon["sprite_path"])        

        # self.player_sprite=pg.transform.scale(base_player_img,(160,160))
        # self.enemy_sprite=pg.transform.flip(base_enemy_img,(160,160),True,False)
        
        # self.player_icon_small = pg.transform.scale(base_player_img, (40, 40))
        # self.enemy_icon_small  = pg.transform.scale(base_enemy_img, (40, 40))       
    ...
    def setup(self, player_mon: dict, enemy_mon: dict,bag)-> None:
        ...
        if "max_hp" in self.enemy_mon:
            self.enemy_mon["hp"] = self.enemy_mon["max_hp"]

        # 依據 sprite_path 產生戰鬥用圖片
        base_player_img = load_img(player_mon["sprite_path"])
        base_enemy_img  = load_img(enemy_mon["sprite_path"])
        ...
        self.enemy_sprite = pg.transform.scale(base_enemy_img, (220, 220))
        ...
        self.enemy_anim_idle = self._build_idle_anim(self.enemy_mon, is_player=False)
        self.enemy_anim_attack = self._build_attack_anim(self.enemy_mon, is_player=False)
        self.enemy_anim = self.enemy_anim_idle
    ...
    def _start_player_attack(self):
        ...
        self.deal_damage(self.player_mon, self.enemy_mon)
        ...
        #     else:
        #         mid = "+0"
            # 這句就是你說的：原本傷害 + / - 被影響的傷害 = 實際傷害
            # self.message_text = (
            #     f"{info['attacker']} attacks! "
            #     f"Damage: {info['base']}{mid} = {info['final']} "
            #     f"(x{info['multiplier']:.1f})  [PRESS SPACE]"
            # )
        else:
            self.message_text = f"{self.player_mon['name']} attacks! [PRESS SPACE]"

        self.state = "player_attack"
        # self.message_text = f"{self.player_mon['name']} attacks {self.dmg}!(PRESS SPACE TO CONTINUE)"
        # # self.message_timer = 3.0
        # self.state = "player_attack"

    def _start_enemy_attack(self):
        if self.enemy_anim_attack is not None:
            self.enemy_anim = self.enemy_anim_attack
            self.enemy_anim.reset()
        self.deal_damage(self.enemy_mon, self.player_mon)
        ...
        #     )
        # else:
            # self.message_text = f"{self.enemy_mon['name']} attacks! [PRESS SPACE]"

        self.state = "enemy_attack"
        # self.message_text = f"{self.enemy_mon['name']} attacks {self.dmg}! (PRESS SPACE TO CONTINUE)"
        # # self.message_timer = 3.0
        # self.state = "enemy_attack"
    ...
    def _enter_victory(self):
        if self.bag is not None:
            for item in self.bag._items_data:
                if item["name"] == "Coins" and item["count"]>0:
                    item["count"] +=50
                if item["name"] == "Pokeball" and item["count"]>0:
                    item["count"] +=2                

        self.state = "victory"
        self.message_text = "You defeated the trainer, You earned 50 coins and 2 Pokeball!"
        self.message_timer = 0.0
        sound_manager.play_bgm("RBY 108 Victory! (Trainer).ogg")

    def _enter_loss(self):
        self.state = "loss"
        self.message_text = None
        self.message_timer = 0.0

    def _get_switch_monsters_(self) ->list[dict]:
        if self.bag  is None:
            return []
        ...
        result = []
        for m in mons:
            # 如果 player_mon 本身也在 bag 裡，你可以選擇要不要跳過
            # 現在簡單全部顯示
            result.append(m)
        return result
    def _open_switch_overlay(self) -> None:
        self.state_before_bag = self.state   # 如果你之後想回到原本狀態可以用
        self.state = "switch"
        self.switch_scroll = 0
        self.switch_selected_mon = None
        self.switch_preview_anim = None
    ...
    def _fit_to_box(self, surf: pg.Surface, max_w: int, max_h: int) -> pg.Surface:
        ...
        if w == 0 or h == 0:
            return surf
        scale = min(max_w / w, max_h / h, 1.0)   # <= 1.0：只縮小不放大（你想放大也可以拿掉 1.0）
    ...
    def update(self, dt):
        ...
        if self.enemy_anim is not None:
    ...
    #    # 還沒 setup 完成，就不要畫 / 不要戰鬥，避免 NoneType
        ...
        if self.state == "bag":
            if self.bag is not None:
                ...
                # 顯示「Healed XXX」「Buff applied」之類的訊息
            ...
            if input_manager.key_pressed(pg.K_ESCAPE) or input_manager.key_pressed(pg.K_SPACE):
                self._close_bag()   
            return
        ...
        if self.state in ("player_attack", "enemy_attack"):
            if input_manager.key_pressed(pg.K_SPACE):
                    ...
                    if self.state == "player_attack":
                        ...
                        if self.enemy_anim_idle is not None:
                            self.enemy_anim = self.enemy_anim_idle
                            self.enemy_anim.reset()
                        # 敵人攻擊文字結束 → 回到玩家行動
                        self.state = "command"
        ...
        if self.state == "command":
            ...
            if input_manager.key_pressed(pg.K_f):
                self.handle_command("Fight")

            elif input_manager.key_pressed(pg.K_b):
                self.handle_command("Bag")
            ...
            # 如果你想要 S = Switch，就加這個
                ...
                self.handle_command("Switch")        


    # def draw_hp_box(self, screen: pg.Surface, mon: dict, *,
    #                 x: int, y: int,
    #                 small_icon: pg.Surface,
    #                 align_right: bool = False) -> None:               
    #     banner_rect = self.info_banner.get_rect(topleft=(x, y))
    #     screen.blit(self.info_banner, banner_rect)

    #     # icon
    #     icon_x = banner_rect.left + 10
    #     icon_y = banner_rect.top + (banner_rect.height - small_icon.get_height()) // 2
    #     screen.blit(small_icon, (icon_x, icon_y))

    #     # 名字
    #     name_surf = self.word_font.render(mon["name"], True, (0, 0, 0))
    #     name_x = icon_x + small_icon.get_width() + 10
    #     name_y = banner_rect.top + 8
    #     screen.blit(name_surf, (name_x, name_y))

    #     # 等級
    #     lvl_surf = self.word_font.render(f"Lv.{mon['level']}", True, (0, 0, 0))
    #     lvl_x = banner_rect.right - lvl_surf.get_width() - 10
    #     lvl_y = banner_rect.top + 8
    ...
    #     bar_h = 16
    #     bar_x = name_x
    #     bar_y = banner_rect.bottom - bar_h - 10
    ...
    #     hp_x = bar_x + bar_w // 2 - hp_text.get_width() // 2
    #     hp_y = bar_y - hp_text.get_height()
    ...
    def draw(self, screen: pg.Surface):
        ...
        if self.enemy_anim is not None:
            img_enemy = self.enemy_anim.get_image()
        else:
            img_enemy = self.enemy_sprite
        ...
        self.draw_hp_box(
            ...
            y=40,
            small_icon=self.enemy_icon_small
        )
        ...
        # if self.state == "intro":
        #     intro_text = f"A trainer wants to battle!(Press SPACE to continue)"
        #     text = self.word_font.render(intro_text, True, (255,255,255))
        #     screen.blit(text, (50, 625))
        # else:
        # #字體
        #     text=self.word_font.render(f"What will {self.player_mon['name']} do?", True,(255,255,255)  )
        #     screen.blit(text,(50,625))
        if self.state in ("victory", "loss"):
            # 半透明遮罩
            overlay = pg.Surface(
            ...
            if self.state == "victory":
                ...
                text_str = "Victory!"
                text = self.word_font.render(self.message_text, True, (255, 255, 255))
                screen.blit(text, (50, 625))
            else:
            ...
            banner_rect.center = (GameSettings.SCREEN_WIDTH // 2,
                                  GameSettings.SCREEN_HEIGHT // 2 - 60)
            banner_sprite.rect = banner_rect
            screen.blit(banner_sprite.image, banner_sprite.rect)

            # 中間文字
            text = self.title_font.render(text_str, True, (0, 0, 0))
            text_x = banner_rect.centerx - text.get_width() // 2
            text_y = banner_rect.centery - text.get_height() // 2
            ...
            return  # 勝敗畫面不用再畫下面的文字 / 按鈕

        # 其他狀態：決定對話框裡顯示什麼文字
        # if self.state in ("player_attack", "enemy_attack") and self.message_text:
        #     text = self.word_font.render(self.message_text, True, (255, 255, 255))
        #     screen.blit(text, (50, 625))
        if self.state in ("player_attack", "enemy_attack") and self.last_damage_info:
            ...
            text = self.word_font.render(
                f"What will {self.player_mon['name']} do?",
                True, (255, 255, 255)
            )
            screen.blit(text, (50, 625))

            hint = self.word_font.render(
                "[F] Fight   [B] Bag   [S] Switch   [ESC] Run",
                True, (200, 200, 200)
            )
            screen.blit(hint, (GameSettings.SCREEN_WIDTH//2-200, 625))


            # 只有在 command 狀態才畫四顆按鈕
            ...
            text = self.word_font.render(
                "A trainer wants to battle! (Press SPACE)",
                True, (255, 255, 255)
            )
            screen.blit(text, (50, 625))
        ...
        if self.state == "switch":
            ...
            if self.switch_selected_mon is not None:
                ...
                # img_rect.center = preview_rect.center
                # screen.blit(img, img_rect.topleft)
                ...
                bar_w = preview_rect.width - 20   # 比預覽框稍微縮一點
                bar_h = 18
                bar_x = preview_rect.x + 10
                bar_y = attr_y + line_gap -90
                ...
                enemy_def = self.enemy_mon.get("defense", 0)  # 或 wild_mon / enemy_mon 看你怎麼命名
